...
import {ResolverCommon} from '../ResolverCommon'
import {mimeTypeIsValid, Content, UrlStream, finishedWithContent} from '../Content'
import {ResolverError, MediaOwnerInfo, MediaFinder, ResolverErrorCode, ResultType} from '../Resolver'
import {resolvers} from '../Public'
        ...
        ResolverCommon.getMimeType(url, tempMediaOwner, process).then(function(mimeType) {
            ...
            var ifMimeTypeIsValidCreate = function(theUrl: string, mType: string, mProcess: ProcessNode): boolean {
                if (mimeTypeIsValid(mType)) {
                    ...
                    finishedWithContent(content, tempMediaOwner, mProcess)
